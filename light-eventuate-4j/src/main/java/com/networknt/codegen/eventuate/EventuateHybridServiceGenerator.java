...
public class EventuateHybridServiceGenerator implements Generator {
    ...
    public void generate(String targetPath, Object model, Any config) throws IOException {
        ...
        transfer(targetPath, ("src.test.resources.config").replace(".", separator), "primary.crt", templates.eventuate.hybrid.primaryCrt.template());
        transfer(targetPath, ("src.test.resources.config").replace(".", separator), "secondary.crt", templates.eventuate.hybrid.secondaryCrt.template());
